...
%% Initialization
load('DataPrepare/DirInfo.mat'); % Load the UrbanNav data folder
...
KF_ON = [1 0];  %[KF, AKF]
FGO_ON = [0 0]; %[FGO, AFGO] 
% KF uncertainty parameters
if contains(CurFolder, 'xiaomi') % xiaomi
    Fixed_KF_Uct = 60; 
    receiver = 'xiaomi';
    tuning_factor_FGO = 1/8; 
    min_uct = 4;
    max_uct = 130;
elseif contains(CurFolder, 'novatel') % novatel
    Fixed_KF_Uct = 20; 
    receiver = 'novatel';
    tuning_factor_FGO = 1/9; 
    min_uct = 1;
    max_uct = 45;
elseif contains(CurFolder, 'ublox') % ublox
    Fixed_KF_Uct = 30;
    receiver = 'ublox';
    tuning_factor_FGO = 1/8; 
    min_uct = 1.5;
        ...
        if ~isnan(sensorData{ii}.GNSSReading)
            ...
            elseif isequal(FGO_ON,[0 1]) 
                ...
                if ~isempty(ml_uct)
                    ...
                    adaptiveUct = ml_uct * tuning_factor_FGO ; 
                else
                    adaptiveUct = max_uct * tuning_factor_FGO; 
                end
...
if isequal(KF_ON, [1 0])
    ...
    savefig([CurFolder,'/vel_enu_kf.fig']);
elseif isequal(KF_ON, [0 1])
    figure;
    %
    subplot(3,1,1);
    ...
    savefig([CurFolder,'/vel_enu_akf.fig']);
end
